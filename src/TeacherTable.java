...
public class TeacherTable extends JFrame implements ActionListener{
    ...
    public void initComponents(){
        ...
        titlelable=new JLabel("Teacher Registration");
    ...
    public void actionPerformed(ActionEvent e) {
        if(e.getSource()==addbutton){
            ...
            if(!fname.isEmpty() && !lname.isEmpty() && !phone.isEmpty() && !course.isEmpty() && !payment.isEmpty()){
                String query="insert into teacher(fname,lname,phone,payment,date,cname) values(?,?,?,?,?,?)";
                try {
                    int ins=Connect.insertteacher(query, fname, lname, phone, payment,course);
            ...
            paymenttf.setText("");
                ...
                if(!id.isEmpty()){
                    ...
                    String query="DELETE FROM teacher WHERE TId="+intid+";";
                    try {
            ...
            if(!fname.isEmpty() && !lname.isEmpty() && !phone.isEmpty() && !course.isEmpty() && !payment.isEmpty() && !id.isEmpty()){
                int intid=Integer.parseInt(id);
                String query="update teacher set tid=?,fname=?,lname=?,phone=?,payment=?,cname=? WHERE tid=?";
                try {
                    int upd=Connect.updateteacher(query, intid, fname, lname, phone, payment,course);
